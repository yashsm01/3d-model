...
export class ShowComponent implements OnInit, AfterViewInit {
  ...
  public availableModels = [
    {
      ...
      file: 'myModel.glb',
      scale: 0.4,
      position: { x: 7.3, y: 5.5, z: 4.3 },
      icon: 'fas fa-box',
      url: 'https://example.com/myModel',
    },
    {
      name: 'Shelf',
      file: 'shelf.glb',
      scale: 1,
      position: { x: 1, y: 7.5, z: 1 },
      icon: 'fas fa-archive',
      url: '',
    },
    {
      name: 'White-house',
      file: 'white-house.glb',
      scale: 20,
      position: { x: 1, y: 7.5, z: 1 },
      icon: 'fas fa-archive',
      url: '',
  ...
  // Add properties for camera settings
  public cameraX: number = 0;
  public cameraY: number = 0;
  public cameraZ: number = 0;
  public cameraAlpha: number = 0;
  public cameraBeta: number = 0;
  public cameraRadius: number = 0;
  ...
  fetchModels(): void {
    this.objectService.getModels().subscribe(
      (models) => {
  ...
  //camera Functions  ----------------------------------------------------------------
  // Add to class properties
  private cameraPositionsAndAngles: {
  ...
  updateCameraPosition(): void {
    if (this.camera instanceof BABYLON.ArcRotateCamera) {
      ...
      const heightToWidthRatio =
        this.innerBoxSize.height / this.innerBoxSize.width;
  ...
  zoomOut() {
    if (this.camera instanceof BABYLON.ArcRotateCamera) {
      this.camera.radius = Math.min(
        this.camera.radius + 5,
        this.camera.upperRadiusLimit ?? Infinity
      );
    } else if (this.camera instanceof BABYLON.UniversalCamera) {
      this.camera.position.z -= 5;
    }
    ...
    this.scene.onBeforeRenderObservable.add(() => {
      ...
      if (this.camera instanceof BABYLON.ArcRotateCamera) {
        ...
        cameraInfo = `
         ...
         Camera Position: X: ${this.camera.position.x.toFixed(
           2
       ...
       `;
        ...
        // this.cameraAlpha = alphaInDegrees;
        // this.cameraBeta = betaInDegrees;
  ...
  storeCurrentCameraPositionAndAngle(): void {
    if (this.camera instanceof BABYLON.ArcRotateCamera) {
      ...
      console.log(
        'Camera position and angle stored:',
  ...
  setupPointerControls(): void {
    ...
    if (canvas) {
      canvas.addEventListener('click', () => {
        if (
          this.camera instanceof BABYLON.UniversalCamera &&
      ...
      switch (pointerInfo.type) {
        case BABYLON.PointerEventTypes.POINTERDOWN:
          ...
          if (this.isModelClicked(pointerInfo)) {
            ...
            if (this.isModelMovementEnabled || this.selectedModel!.isMovable)
        ...
        case BABYLON.PointerEventTypes.POINTERMOVE:
          if (
            this.isDraggingModel &&
            this.selectedModel &&
            this.isModelMovementEnabled
          ) {
            if (this.isArcRotateCamera) {
  ...
  addToSelectedModels(model: BABYLON.AbstractMesh): void {
    if (!this.selectedModels.includes(model)) {
      ...
      const sameModelCount = this.selectedModels.filter((m) =>
        m.name.startsWith(model.name.split('_')[0])
      ).length;

      const stackHeight =
        sameModelCount * (this.modelSizeY + this.STACK_SPACING);
      const stacksInHeight = Math.floor(
        availableHeight / (this.modelSizeY + this.STACK_SPACING)
      );
      const stackDepth =
        Math.floor(stackHeight / availableHeight) *
        (this.modelSizeZ + this.STACK_SPACING);
      const stacksInDepth = Math.floor(
        availableDepth / (this.modelSizeZ + this.STACK_SPACING)
      );
      const stackWidth =
        Math.floor(
          (stackHeight / availableHeight) * (stackDepth / availableDepth)
        ) *
        (this.modelSizeX + this.STACK_SPACING);
      ...
      if (stackWidth < availableWidth) {
        model.position.x =
          -this.innerBoxSize.width / 2 + this.WALL_MARGIN + stackWidth;
        model.position.y =
          -this.innerBoxSize.height / 2 +
          this.WALL_MARGIN +
          (stackHeight % availableHeight);
        model.position.z =
          -this.innerBoxSize.depth / 2 +
          this.WALL_MARGIN +
          (stackDepth % availableDepth);
  ...
  updateSelectedModelsList(): void {
    const selectedModelsListElement =
      document.getElementById('selectedModelsList');
    if (selectedModelsListElement) {
      selectedModelsListElement.innerHTML = this.selectedModels
  ...
  private setCameraPositionAndAngle(position: BABYLON.Vector3): void {
    if (this.camera instanceof BABYLON.ArcRotateCamera) {
      ...
      this.camera.setPosition(
        new BABYLON.Vector3(position.x + 10, position.y + 10, position.z + 10)
      );
      this.camera.alpha = alpha;
  ...
  addModel(
    glbUrl: string,
    ration: number,
    defaultModelSize: DefaultModelSize = this.DEFAULT_MODEL_SIZE,
    url: string
    ...
    return new Promise((resolve, reject) => {
      ...
      BABYLON.SceneLoader.ImportMesh(
        ...
        (meshes) => {
          ...
          glbModel.position = new BABYLON.Vector3(
            0,
            -this.innerBoxSize.height / 2,
            5
          );
          ...
          while (!validPosition && attempts < maxAttempts) {
            ...
            for (const existingModel of this.glbModels) {
              if (this.checkCollision(glbModel, existingModel)) {
                ...
                if (
                  glbModel.position.x >
                  this.innerBoxSize.width / 2 - this.WALL_MARGIN
                ) {
                  glbModel.position.x =
                    -this.innerBoxSize.width / 2 + this.WALL_MARGIN;
                  ...
                  if (
                    glbModel.position.z >
                    this.innerBoxSize.depth / 2 - this.WALL_MARGIN
                  ) {
                    glbModel.position.z =
                      -this.innerBoxSize.depth / 2 + this.WALL_MARGIN;
          ...
          console.log(
            `Model Size - X: ${this.modelSizeX}, Y: ${this.modelSizeY}, Z: ${this.modelSizeZ}`
          );
          resolve(glbModel);
        },
        null,
    ...
    if (pickResult?.hit) {
      ...
      for (const glbModel of this.glbModels) {
        if (
          ...
          (pickResult.pickedMesh === glbModel ||
            pickResult.pickedMesh.isDescendantOf(glbModel))
        ) {
          this.selectedModel = glbModel;
          return true;
        }
      ...
      if (pickInfo.hit) {
        ...
        for (const model of this.glbModels) {
          if (
            ...
            this.checkCollision(this.selectedModel, model)
          ) {
  ...
  zoomInOnModel(): void {
    if (
      !this.selectedModel ||
      !(this.camera instanceof BABYLON.ArcRotateCamera)
    )
      return;

    const boundingInfo = this.selectedModel.getBoundingInfo();
    const center = boundingInfo.boundingBox.centerWorld;
    const radius = boundingInfo.boundingBox.extendSizeWorld.length() * 30;
  ...
  applyCameraSettings(): void {
    if (this.camera instanceof BABYLON.ArcRotateCamera) {
      this.camera.setPosition(
        new BABYLON.Vector3(this.cameraX, this.cameraY, this.cameraZ)
      );
      // this.camera.alpha = BABYLON.Tools.ToRadians(this.cameraAlpha);
      // this.camera.beta = BABYLON.Tools.ToRadians(this.cameraBeta);
  ...
  rotateModel(axis: string, amount: number): void {
    ...
    if (!this.selectedModel.rotationQuaternion) {
      this.selectedModel.rotationQuaternion =
    ...
    switch (axis) {
      case 'x':
        rotationQuaternion.multiplyInPlace(
          BABYLON.Quaternion.RotationAxis(BABYLON.Axis.X, rotationAmount)
        );
        break;
      case 'y':
        rotationQuaternion.multiplyInPlace(
          BABYLON.Quaternion.RotationAxis(BABYLON.Axis.Y, rotationAmount)
        );
        break;
      case 'z':
        rotationQuaternion.multiplyInPlace(
          BABYLON.Quaternion.RotationAxis(BABYLON.Axis.Z, rotationAmount)
        );
        break;
    }
    ...
    console.log(
      `Rotation Quaternion - X: ${rotationQuaternion.x}, Y: ${rotationQuaternion.y}, Z: ${rotationQuaternion.z}, W: ${rotationQuaternion.w}`
    );
  }
  ...
  scaleModel(axis: string, factor: number): void {
    ...
    console.log(
      `Scaling - X: ${this.selectedModel.scaling.x}, Y: ${this.selectedModel.scaling.y}, Z: ${this.selectedModel.scaling.z}`
    );
  }
  ...
  checkCollision(
    model1: BABYLON.AbstractMesh,
    model2: BABYLON.AbstractMesh
  ): boolean {
    ...
    return (
      Math.abs(pos1.x - pos2.x) <
        (size1.x + size2.x) / 2 + this.MODEL_SPACING &&
      Math.abs(pos1.y - pos2.y) <
        (size1.y + size2.y) / 2 + this.MODEL_SPACING &&
    ...
    if (this.selectedModel) {
      ...
      for (const [side, distance] of Object.entries(distances)) {
        const line = BABYLON.MeshBuilder.CreateLines(
          `line-${side}`,
          {
            points: [modelPosition, this.getWallPosition(side, modelPosition)],
            colors: [
        ...
        const vertices = line.getVerticesData(
        ...
        if (vertices) {
          ...
          transformNode = new BABYLON.TransformNode(
            `transformNode-${side}`,
            this.scene
          );
  ...
  getWallPosition(
    side: string,
    modelPosition: BABYLON.Vector3
  ): BABYLON.Vector3 {
    switch (side) {
      case 'left':
        return new BABYLON.Vector3(
          -this.innerBoxSize.width / 2,
          modelPosition.y,
          modelPosition.z
        );
      case 'right':
  ...
  private openUrlModel(model: BABYLON.AbstractMesh): void {
    ...
    if (!this.isModelMovementEnabled && this.selectedModel) {
      const modelData = this.availableModels.find(
        (model) => model.file === this.selectedModel?.name.split('_')[0]
      );
  ...
  private createModelTooltip(): void {
    ...
    if (!this.modelTooltip) {
      ...
      this.modelTooltip.background = new BABYLON.Color4(
        0,
        0,
        0,
        0.7
  ...
  togglePlayMode(): void {
    if (this.cameraPositionsAndAngles.length === 0 && this.isPlayMode == false)
      return;
  ...
  startPlayMode(): void {
    ...
    const moveToNextPosition = () => {
      ...
      if (this.camera instanceof BABYLON.ArcRotateCamera) {
        ...
        this.animateUniversalCameraPosition(
          this.camera,
          position,
          moveToNextPosition
        );
  ...
  private animateUniversalCameraPosition(
  ...
  moveToNextCameraPosition(): void {
    if (this.currentCameraIndex < this.cameraPositionsAndAngles.length - 1) {
      ...
      const { position, alpha, beta } =
        this.cameraPositionsAndAngles[this.currentCameraIndex];

      // Adjust sensitivity
      if (this.camera instanceof BABYLON.ArcRotateCamera) {
        this.camera.wheelDeltaPercentage = 0.01; // Lower value for reduced scroll sensitivity
        this.camera.inertia = 0.5; // Smooth deceleration
      }
  ...
  private animateScrollerCameraPosition(
    ...
    const animationSpeed = 2; // Lower value for smoother, slower animations
    ...
    // Clear previous animations and add new ones
    camera.animations = [animationPosition];

    // Alpha and Beta animations for ArcRotateCamera
    if (
      ...
      // Add alpha and beta animations
      camera.animations.push(animationAlpha, animationBeta);
    }

    // Begin animation with reduced speed for smoother transitions
  ...
  exportModels(): void {
    ...
    const exportName =
      'modelLayout_' + new Date().toISOString().slice(0, 10) + '.json';
  ...
  importModels(): void {
    if (this.selectedModels.length > 0) {
      if (
        !confirm(
          'Importing will remove all existing models. Do you want to continue?'
        )
      ) {
        return;
      }
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          ...
          try {
            ...
            alert(
              'Error loading file. Please ensure it is a valid model layout file.'
            );
          }
        };
  ...
  importCameraPositions(): void {
    if (this.cameraPositionsAndAngles.length > 0) {
      if (
        !confirm(
          'Importing will remove Existing Angles. Do you want to continue?'
        )
      ) {
        return;
      }
      // Clear existing camera positions and angles
      this.cameraPositionsAndAngles = [];
    }
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          ...
          try {
            ...
            this.cameraPositionsAndAngles = this.cameraPositionsAndAngles.map(
              (data: any) => {
                return {
